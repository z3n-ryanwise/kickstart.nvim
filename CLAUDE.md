...
- `<leader>gb` -> Toggle git blame (plugin/git.lua)
- `<leader>gy` -> Yank GitHub permalink for current line/selection (plugin/git.lua)
- `gd` -> LSP: goto definition (plugin/lsp.lua)
- `gD` -> LSP: goto declaration (plugin/lsp.lua)
- `gr` -> LSP: find references (plugin/lsp.lua)
- `gI` -> LSP: goto implementation (plugin/lsp.lua)
- `gy` -> LSP: goto type definition (plugin/lsp.lua)
